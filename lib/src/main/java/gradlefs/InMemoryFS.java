...
public class InMemoryFS extends FuseStubFS {
    ...
    private class MemoryFile extends MemoryPath {
        ...
        public int readChecksum(Pointer value, long size) {
            try {
                MessageDigest md5 = MessageDigest.getInstance("md5");
                md5.update(contents.array());
                byte[] digest = md5.digest();
                value.put(0, digest, 0, digest.length);
    ...
    public int getxattr(String path, String name, Pointer value, long size) {
        if ("user.checksum".equals(name)) {
            if (value == null) {
                return 16; // attribute size
            }
            MemoryPath p = getPath(path);
            if (p instanceof MemoryFile) {
                return ((MemoryFile)p).readChecksum(value, size);
            }
        }
